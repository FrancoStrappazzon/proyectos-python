...
# of elements in nums which are not equal to val.Consider the number of elements in nums which are not equal to val be k, to get accepted, you need to do the
# following things:
...
def removeElements(nums, val):
   ...
   #actualizo la lista nums
    nums[:] = nueva_lista
    k = len(nums)
    return k

nums = [1,2,4,8,9,12,4,66,123,4]
val =4
